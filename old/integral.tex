...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.5]{df=f'dx.jpg}
    \caption{Caption}
...
\boxed{\textbf{Таким образом, интегрируют не функцию $f(x)$, а  дифференциальную 
...
\begin{lemma}\label{int_of_prime} Интеграл от формы $\frac{Ax + B}{(x^2 + ax + b)^n}\mathrm{d}x$ выражается через рациональные функции и функции $\ln$, $\arctan$.
\end{lemma}
\begin{proof}
